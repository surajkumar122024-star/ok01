...
export default function JpgToPngPage() {
  ...
  const handleConvert = async () => {
    ...
    try {
      const blob = await processImage(selectedFile, { format: 'image/png' });
  ...
  const handleDownload = () => {
    ...
    a.href = url;
    a.download = `${selectedFile?.name.split('.')[0] || 'image'}.png`;
    a.click();
  ...
  return (
    <ImageToolLayout 
      ...
      description="Convert your JPEG images to PNG format instantly. Perfect for preserving transparency or high quality."
    >
          ...
          <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
            <div className="flex items-center justify-center gap-8 py-8 px-4 bg-muted/30 rounded-xl border">
              <div className="text-center">
                <div className="text-2xl font-bold text-muted-foreground mb-1">JPG</div>
            ...
            {processedBlob && (
              <div className="p-4 bg-primary/5 border border-primary/20 rounded-lg text-center">
                <p className="text-sm text-primary font-medium">
                  Conversion complete. Your PNG file is ready for download.
                </p>
              </div>
            )}
          </div>
